...
tclsh [options] script.l args

=head1 DESCRIPTION

XXX Larry to write the big-picture words. XXX

=head1 RUNNING L PROGRAMS

You can run an L program from the command line.

    tclsh [options] progname.l
...
the current end, the un-assigned elements are assigned undef:
...
larger complex structures.  To concantenate two arrays, you could do
this:
...
modeled after the standard C library.
...
=item int frename(string oldpath, string newpath)
...
=item void push(T &array[], T element)
...
=item T[] sort([decreasing: | increasing:], T[] array)

=item T[] sort([integer: | real: | ascii:], T[] array)

=item T[] sort(command: compar, T[] array)
...
full-blown object-oriented programming (there is no inheritence), but
...
declare an C<object> of C<mylcass>:

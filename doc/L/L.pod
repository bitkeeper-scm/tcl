...
From C, L gets C syntax, types (int, float, string), complex types
(arrays, structs).
...
L provides a set of builtin functions, drawn from Perl and/or the 
standard C library.
...
Tcl code may be intermixed if needed.  More importantly, it means that
...
L is open source under the same license as the Tcl/TK license (BSD like).

=head1 RUNNING L PROGRAMS

You can run an L program from the command line.

    L [options] progname.l
...
or class declarations.  Statements not enclosed in functions are said
to be at the C<top level> and are executed in the order they are
written.  There is no need to have a C<main()> function, but if one is
...
If an initial value is omitted, an appropriate one is supplied.  Ints
get 0, floats get 0.0, strings get "", arrays and hashes start out
empty, and each struct member gets an initial value appropriate for
its type.
...
The special pre-defined identifier C<END> is index of the last element
of an array, unless the array is empty and then END is undef:
...
the array is empty, END will be undef, and using undef as an array
...
You can remove an element from an array but only from the end:

    birds = { "robin", "dove", "cardinal" };
    next = pop(&birds);   // next gets "cardinal"
...
Sometimes you don't want L to do type checking. (XXX need example.)
...
function names B<cannot> begin with an underscore (_).  However, a
special kind of function called a C<pattern function> can end with
C<_*>; these are described later.
...
=head2 Tcl and Pattern Functions

L has something called a C<pattern function> which is an exception to
some of these rules.  A pattern function call is one whose name is
capitalized and has an underscore (except at the end), like this:

    Foo_bar(a, b, c);

Pattern function calls obey two special rules.  First, if C<Foo_*>
is declared as a function, then the compiler translates the call
to this:

    Foo_*("bar", a, b, c);

Second, if C<Foo_*> does not exist, then the call becomes

    <a>(bar, b, c);

where <a> means that the I<value> of C<a> is the function name
to call.

XXX explain why these are useful. XXX

L also has a feature to facilitate calling tcl functions where options
are passed as C<-option>:
...
=head1 CONDITIONAL AND LOOP STATEMENTS
...
The braces are not required, but with nested C<if/else>'s they are
advised so it is clear which C<else> belongs to which C<if>.  If
braces are omitted, an C<else> binds to the closest C<if>.
...
string is empty, END is undef.  Valid only inside of a string or array
subscript.  Read only.
...
=item void assert(int condition)
...
=item string basename(string path)

Return the file portion of a path name.

=item string caller(int frame)
...
=item int chdir(string dir)

Change directory to dir.
Return 0 on success, -1 on error.

=item int chmod(string path, string permissions)
...
=item int chown(string owner, string group, string path)
...
=item void die(string message)

Output the message to stderr and exit 1. If message does not have a
trailing newline, append " in <filename> at line <linenum>\n".

=item string dirname(string path)

Return the directory portion of a pathname.

=item int exists(string path)

Return 1 if the given path exists or 0 if it does not exist.

=item int fclose(FILE f)

Close an open FILE handle.
Return 0 on success, -1 on error.

=item int fgetline(FILE f, string &buf)

Get the next line from a FILE handle and store it in &buf.  Return 1
if a line was read or 0 if not.

=item FILE fopen(string path, string mode)
...
=item int fprintf(FILE f, string fmt, ...args)
...
=item string[] getdir(string dir, string pattern)
...
=item type[] sort(type[] array)

=item type[] sort([decreasing: | increasing:], type[] array)

=item type[] sort([integer: | real: | ascii:], type[] array)

=item type[] sort(command: compar, type[] array)
...
=item string sprintf(string fmt, ...args)

Format arguments and return a formatted string like sprintf(3).
Return undef on error.

=item int stat(string path, struct stat &buf)

Call stat(2) on C<path> and place the information in C<buf>.
Return 0 on success, -1 on error.
See the lstat() command for the definition of C<struct stat>.

=item string stdio_getLastError(void)

Return the last error from an fclose(), fopen(), popen(), or system() call.

=item int strchr(string s, string c)

Return the first index of c into s, or -1 if c is not found.

=item int streq(string a, string b)

Return 1 if the two strings are equal, else return 0.

=item string[] split(string s)

=item string[] split(string s, /regexp/)

=item string[] split(string s, /regexp/, int limit)

=item string[] split(string s, string sep)

=item string[] split(string s, sep, int limit)
...
=head2 Manipulating complex structures
...
L's regular expression support is reminiscent of Perl's.

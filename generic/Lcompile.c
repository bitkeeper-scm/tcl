...
private void L_free_ast(L_ast_node *ast);
...
private int L_push_set_of_indices(L_expression *expr, L_type *type,
...
private void L_write_index(L_symbol *var, L_expression *index,
  L_expression *expr, L_expression *rval, int post_incr_p);
private Tcl_HashTable *L_typedef_table(void);
private L_expression *reference_mangle(char *name);
private int param_passed_by_name_p(L_variable_declaration *p);
private L_symbol *L_get_local_symbol(L_expression *name, int error_p);
...
private int LDumpAstNodes(L_ast_node *node, void *data, int order);
private int push_parameters(char *funcname, L_expression *parameters);
private void L_push_pointer(L_expression *lval);
private L_compile_frame *enclosing_loop_frame(int include_switch_p);
private void fixup_jumps(CompileEnv *envPtr, JumpOffsetList *jumps,
			int targetOffset);
private Proc *Begin_Proc(void);
private void Finish_Proc(Proc *procPtr, char *name);
private int instruction_for_l_op(int op);
private void regsub_for_assignment(char *varName, int varIndex,
	L_expression *regexp);
private void track_lineInfo(int codeOffset, int srcOffset, int len);
private int push_regexp_modifiers(L_expression *regexp);
...
private void L_compile_push(L_expression *expr);
...
private int L_compile_toplevel_statements(L_toplevel_statement *stmt);
private void maybeFixupEmptyCode(L_compile_frame *frame);
private void L_compile_function_decl(L_function_declaration *fun);
private void L_compile_struct_decl(L_type *decl);
private void L_compile_global_decls(L_variable_declaration *decl);
private void L_compile_statements(L_statement *stmt);
private void L_compile_variable_decls(L_variable_declaration *var);
private void L_compile_expressions(L_expression *expr);
...
private L_type *L_compile_index(L_type *index_type, L_expression *index);
private void L_return(int value_on_stack_p);
private L_symbol *L_make_symbol(L_expression *name, L_type *type,
    int localIndex);
private void L_compile_parameters(L_variable_declaration *param);
private void L_compile_defined(L_expression *lval);
private void L_compile_block(L_block *block);
private void L_compile_break(L_statement *stmt);
private void L_compile_continue(L_statement *stmt);
private void L_compile_incdec(L_expression *expr);
private void L_compile_unop(L_expression *expr);
private void L_compile_interpolated_string(L_expression *expr);
private void L_compile_assignment(L_expression *expr);
private void L_compile_short_circuit_op(L_expression *expr);
private void L_compile_twiddle(L_expression *expr);
private void L_push_variable(L_expression *expr);
private void l_push_literal(L_expression *expr);
private void L_compile_binop(L_expression *expr);
	...
	L_type *type)
{
	...
	switch (type->kind) {
	    ...
	    case L_TYPE_ARRAY:
		if (init->value &&
		    ((L_ast_node *)init->value)->type == L_NODE_INITIALIZER) {
			L_PUSH_STR("::list");
		...
		if (init->value &&
			...
			for (i = (L_initializer *)init->value, count = 0;
				...
				if (!i->key) {
					L_errorf(i,
					    "Keys are required for hash initializers.");
				} else {
...
L_compile_expressions(L_expression *expr)
{
	...
	switch (expr->kind) {
		...
		param_count =
		    push_parameters(symbol ? NULL : expr->a->u.string,
			expr->b);
		L_INVOKE(param_count+1);
		if (!symbol) {
			L_check_arg_count(expr->a->u.string,
			    param_count, expr);
		}
		...
		track_lineInfo(startOffset, ((L_ast_node *)expr->a)->offset,
		    (((L_ast_node *)expr)->offset -
			((L_ast_node *)expr->a)->offset)+1);
		break;
...
private void
L_compile_unop(L_expression *expr)
{
	...
	switch (expr->op) {
		...
		if ((expr->a->kind == L_EXPRESSION_VARIABLE) &&
			...
			/* XXX: this duplicates some code from
			   L_push_variable... maybe 'twould be better
			   to parameterize L_push_variable? */
			L_symbol *var;
...
private void
L_compile_binop(L_expression *expr)
{
	L_type *type;
...
L_compile_twiddle(L_expression *expr)
{
	...
	if  (!submatchCount && rtype) {
		...
		unless (simple) L_compile_expressions(regexp->a);
		/* the target string */
		L_compile_expressions(expr->a);
		if (simple) {
			if (exact && !nocase) {
...
L_compile_foreach_loop_array(L_foreach_loop *loop)
{
	...
	if (TclFixupForwardJumpToHere(lframe->envPtr, &jumpFalseFixup, 127)) {
		...
		if (jumpBackDist > 120) {
			...
			TclUpdateInstInt1AtPc(INST_JUMP1,
			    -jumpBackDist, jumpPc);
		}
	}
	fixup_jumps(lframe->envPtr, break_jumps,
	    CurrentOffset(lframe->envPtr));
}
...
L_compile_foreach_loop_hash(L_foreach_loop *loop)
{
	...
	if (loop->value) {
		if (!(valueVar = L_get_local_symbol(loop->value, TRUE))) {
			return;
		}
	}
...
L_push_variable(L_expression *expr)
{
	...
	if (!expr->indices) {
		...
		flags =
		    L_push_set_of_indices(expr, var->type, &depth, &counts);
...
private L_symbol *
L_make_symbol(L_expression *name, L_type *type, int localIndex)
{
	int new;
...
L_type *
L_lookup_typedef(L_expression *name, int error_p)
{
	Tcl_HashEntry *hPtr = NULL;
...
void
L_store_typedef(L_expression *name, L_type *type)
{
	int new;
	Tcl_HashEntry *hPtr;
	L_type *t;
...
void
L_pattern_store_name(L_expression *name)
{
	int new;
    ...
    int len)			/* The length of the source of the command. */
{
	...
	while ((cmdIndex > 0) &&
		...
		envPtr->cmdMapPtr[cmdIndex].codeOffset))
		{
			...
			envPtr->cmdMapPtr[cmdIndex] =
			    envPtr->cmdMapPtr[cmdIndex-1];
...
fresh_include_p(Tcl_Interp *interp, const char *file)
{
	...
	if (!new) L_trace("file %s already included", normalizedPath);
